...
public static class DatabaseSeeder
{
    public static async Task SeedAsync(ApplicationDbContext context)
    {
        ...
        /* if (await context.Set<RedeEscolar>().AnyAsync())
         ...
         // Seed de Usuários (apenas para desenvolvimento)
         if (!await context.Set<SistemaGestaoEscolar.Auth.Domain.Entities.User>().AnyAsync())
         {
             var adminUser = new SistemaGestaoEscolar.Auth.Domain.Entities.User(
                 "Admin",
                 "User",
                 new SistemaGestaoEscolar.Auth.Domain.ValueObjects.Email("admin@escola.com"),
                 new SistemaGestaoEscolar.Auth.Domain.ValueObjects.Password("Admin321!"),
                 SistemaGestaoEscolar.Auth.Domain.ValueObjects.UserRole.SuperAdmin
             );

             context.Set<SistemaGestaoEscolar.Auth.Domain.Entities.User>().Add(adminUser);
             await context.SaveChangesAsync();
         }*/
    }
}