...
public class NomeProfessor : ValueObject
{
    ...
    private NomeProfessor() { } // Para EF Core

    public NomeProfessor(string valor)
    {
        if (string.IsNullOrWhiteSpace(valor))
            throw new ArgumentException("Nome do professor não pode ser vazio", nameof(valor));

        if (valor.Length < 2)
            throw new ArgumentException("Nome do professor deve ter pelo menos 2 caracteres", nameof(valor));

        if (valor.Length > 100)
            throw new ArgumentException("Nome do professor não pode ter mais de 100 caracteres", nameof(valor));

        // Validar se contém apenas letras, espaços e acentos
        if (!System.Text.RegularExpressions.Regex.IsMatch(valor, @"^[a-zA-ZÀ-ÿ\s]+$"))
            throw new ArgumentException("Nome do professor deve conter apenas letras e espaços", nameof(valor));
    ...
    public static implicit operator string(NomeProfessor nome) => nome?.Valor ?? string.Empty;
    public static implicit operator NomeProfessor(string valor) => new(valor);
}